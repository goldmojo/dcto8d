...
char path[3][TEXT_MAXLENGTH];   //r�pertoires des fichiers k7, fd, memo
...
//Cr�ation d'une surface contenant un texte////////////////////////////////////
...
void Initbuttonsurfaces()
{
 ...
 {
  if(bouton[i].name[0] == 0) continue; //bouton non cr��
  w = bouton[i].w;
  h = bouton[i].h;
...
void Drawk7index()
{
 ...
 #ifdef __GCW0__
 rect.x = 2; rect.w = 25; rect.y = 2; rect.h = 15;
 #else
...
void Drawstatusbar()
{
 ...
 #ifdef __GCW0__
 rect.y = 2; rect.h = 15; rect.w = 50;
 rect.x = 49;  Drawtextbox(statusbar, k7name, rect, 0, blanc, -1);
 rect.x = 121; Drawtextbox(statusbar, fdname, rect, 0, blanc, -1);
 rect.x = 201; Drawtextbox(statusbar, memoname, rect, 0, blanc, -1);
 #else
